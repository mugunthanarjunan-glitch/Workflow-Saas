...
class WorkflowEngine {
  ...
  static async executeStep(execution, stepId, iteration = 0) {
    ...
    if (rules.length === 0) {
      ...
      execution.logs.push({
        ...
        action: 'no_rules',
        status: 'completed',
        message: 'No rules found. Workflow completed.',
        timestamp: new Date(),
      });
      await execution.save();
      return;
  ...
  static async handleTaskStep(execution, step, startTime) {
    ...
    // If a specific user is assigned, create a task and pause
    if (step.metadata?.assigned_to) {
      const assignee = await User.findById(step.metadata.assigned_to);
  ...
  static async resumeAfterApproval(execution, step, approved) {
    ...
    if (rules.length === 0) {
      ...
      execution.logs.push({
        ...
        action: 'no_rules',
        status: 'completed',
        message: 'No rules after approval. Workflow completed.',
        timestamp: new Date(),
      });
      await execution.save();
      return;
